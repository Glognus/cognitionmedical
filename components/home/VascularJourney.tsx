							...
							{prefersReducedMotion ? (
								<div className="flex items-center justify-center h-full">
									<div className="text-center p-8">
										...
										<p className="text-text-muted text-sm">Neurovascular navigation technology</p>
									</div>
								</div>
							) : (
								<>
									<VascularNetwork className="absolute inset-0" progress={progress} />
									<CatheterPath className="absolute inset-0" progress={progress} />
								</>
							)}
						</div>
					</div>
					...
					<div className="w-1/2 h-full flex items-center">
						<div className="w-full max-w-lg px-8 lg:px-12">
							{panels.map((panel, index) => {
								...
								const panelProgress = Math.max(0, Math.min(1, (progress - index * 0.25) * 4));
