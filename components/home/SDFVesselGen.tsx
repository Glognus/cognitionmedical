...
export function generateClotGeometryForVessel(
	...
	for (let i = 0; i < numSlices; i++) {
		...
		for (let j = 0; j < numEdgeBumps; j++) {
			...
			const offset = right
				.clone()
				.multiplyScalar(Math.cos(angle) * bumpDist)
...
export function generateVesselGeometry(
	...
	const _strength = 0.5; // Blob strength
	const _subtract = 12; // Isolation level? No, subtract is boolean or strength modifier.
